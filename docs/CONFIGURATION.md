...
| --- | --- | --- |
| `PICTARIA_IMAGE_TAG` | `1.4.0` | Published container image tag selected by `docker-compose.yml`. Image tags omit the `v` used by Git release refs (`1.4.0` versus `v1.4.0`). |
...
| `OPENAI_COMPATIBLE_TIMEOUT_MS` | `300000` | Per-request timeout for the generic endpoint, in milliseconds. Raise it for slow local vision models — large MoE builds on modest hardware can need far longer than the 5-minute default per photo. A photo that exceeds it is reported as a timeout and retried on a later run. |
| `OPENAI_COMPATIBLE_MAX_TOKENS` | `8192` | Output token cap for the generic endpoint. `none` disables the cap. Too low a cap truncates long responses mid-JSON, which shows up as "bad JSON from model" failures. |
| `OPENAI_COMPATIBLE_JSON_SCHEMA` | `false` | Send `response_format: json_schema` (Pictaria's full schema) instead of the portable `json_object`, so a supporting server (llama.cpp, LM Studio) enforces the schema while decoding — bounding output size and guaranteeing schema-valid JSON. Enable only for endpoints known to support the dialect; `json_object` remains the portable default. String length limits are projected down to what llama.cpp's grammar compiler accepts; the prompt still carries the full schema and local validation still enforces the true limits. |
